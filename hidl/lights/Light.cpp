...
Light::Light(std::ofstream&& backlight1, std::ofstream&& backlight2)
    ...
    , mSequencerLoad(LEDS_SEQ_LOAD_FILE)
    , mLightsARGB(0)
    , mLightsBrightness(0)
    , mLightsDelayOn(-1)
    , mLightsDelayOff(-1)
...
void Light::setLightsProgram(leds_pupdate leds_program_update, int leds_program_target,
    ...
    switch (flashMode) {
        ...
        case Flash::TIMED:
            if (leds_program_target != LEDS_SEQ_BLINK_NONE) {
                ...
                if (mSequencer1State != LEDS_SEQ_ENABLED) {
                    mSequencer1Mode.write(LEDS_SEQ_MODE_ACTIVATED.c_str(), LEDS_SEQ_MODE_ACTIVATED.length());
                    mSequencer1Run.write(LEDS_SEQ_RUN_ACTIVATED.c_str(), LEDS_SEQ_RUN_ACTIVATED.length());
        ...
        default:
            ...
            mSequencer1Run.write(LEDS_SEQ_RUN_DISABLED.c_str(), LEDS_SEQ_RUN_DISABLED.length());
            mSequencer1Mode.write(LEDS_SEQ_MODE_DISABLED.c_str(), LEDS_SEQ_MODE_DISABLED.length());
            mSequencer1State = LEDS_SEQ_DISABLED;
            break;
    }
    ...
    if (mSequencer2State != LEDS_SEQ_DISABLED) {
        mSequencer2Run.write(LEDS_SEQ_RUN_DISABLED.c_str(), LEDS_SEQ_RUN_DISABLED.length());
        mSequencer2Mode.write(LEDS_SEQ_MODE_DISABLED.c_str(), LEDS_SEQ_MODE_DISABLED.length());
        mSequencer2State = LEDS_SEQ_DISABLED;
    }
    if (mSequencer3State != LEDS_SEQ_DISABLED) {
        mSequencer3Run.write(LEDS_SEQ_RUN_DISABLED.c_str(), LEDS_SEQ_RUN_DISABLED.length());
        mSequencer3Mode.write(LEDS_SEQ_MODE_DISABLED.c_str(), LEDS_SEQ_MODE_DISABLED.length());
...
void Light::writeLightsProgram(int leds_program_target, int delayOn, int delayOff) {
    ...
    bytes = snprintf(buffer, sizeof(buffer), LEDS_SEQ_LOAD_PROGRAM.c_str(),
            ...
            leds_program_target);
    mSequencerLoad.write(buffer, bytes);
